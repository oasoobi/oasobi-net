              ...
              <Tabs defaultValue="description">
                ...
                <TabsContent value="downloads" className="select-none">
                  <div className="pb-4 pt-3">
                    ...
                    {releases.map((release, index) => (<div key={release.id}><DownloadCard release={release} isLatest={index === 0}/>{index == 0 ? (<p className="mt-6 text-md font-bold">以降のバージョンは過去のバージョンです。最新版で動作しない可能性があります。</p>):(<></>)}</div>))}
                  </div>
                </TabsContent>
              </Tabs>
            </div>
